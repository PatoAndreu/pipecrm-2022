...
export default function useActivities() {
  ...
  const { activities, task, showModal, isEditing, minimize, showAssociations, activeTab } = storeToRefs(activitiesStore)
  ...
  const completedActivities = computed<IActivity[]>(() => {
    return activities?.value?.filter((a) => {
      if (a.type !== "note") return a.completed && !a.pinned && !a.delayed
      return !a.pinned
    })
  })

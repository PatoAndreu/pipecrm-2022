...
export const useContactsStore = defineStore("contacts", {
  ...
  getters: {
    ...
    filteredContacts(state) {
      ...
      const primerDiaDeLaSemanaPasada: string = dayjs()
        .subtract(1, "weeks")
        .startOf("week")
        .format(formatDate)
      const ultimoDiaDeLaSemanaPasada: string = dayjs()
        .subtract(1, "weeks")
        .endOf("week")
        .format(formatDate)
      ...
      const primerDiaDelMesPasado: string = dayjs()
        .subtract(1, "months")
        .startOf("month")
        .format(formatDate)
      const ultimoDiaDelMesPasado: string = dayjs()
        .subtract(1, "months")
        .endOf("month")
        .format(formatDate)
      ...
      switch (state.filterDate?.id) {
        case 1:
          return state.contacts.filter(
            (contact) => dayjs(contact.createdAt).format(formatDate) === hoy
          )
        case 2:
          return state.contacts.filter(
            (contact) => dayjs(contact.createdAt).format(formatDate) === ayer
          )
        case 3:
          return state.contacts.filter(
            (contact) =>
              dayjs(contact.createdAt).format(formatDate) <= hoy &&
              dayjs(contact.createdAt).format(formatDate) >= primerDiaDeLaSemana
          )
        case 4:
          return state.contacts.filter(
            (contact) =>
              dayjs(contact.createdAt).format(formatDate) >= primerDiaDeLaSemanaPasada &&
              dayjs(contact.createdAt).format(formatDate) <= ultimoDiaDeLaSemanaPasada
          )
        case 5:
          return state.contacts.filter(
            (contact) =>
              dayjs(contact.createdAt).format(formatDate) >= primerDiaDelMes &&
              dayjs(contact.createdAt).format(formatDate) <= ultimoDiaDelMes
          )
        case 6:
          return state.contacts.filter(
            (contact) =>
              dayjs(contact.createdAt).format(formatDate) >= primerDiaDelMesPasado &&
              dayjs(contact.createdAt).format(formatDate) <= ultimoDiaDelMesPasado
          )
      }
